    ...
    while(!f.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            for (int i = 0; i < points.size(); ++i) {
                ...
                if(pt[0] == pt0[0] && pt[1] == pt0[1] && pt[2] == pt0[2])
                {
                    isHistoryPoint = true;
                }
            }
            ...
            for (int i = 0; i < points.size(); ++i) {
                Eigen::Vector4d pt = points[i];
                if(pt[0] == pt1[0] && pt[1]==pt1[1] && pt[2]==pt1[2])
                {
                    isHistoryPoint = true;
                }
            }
...
//    std::cout << Qwb.coeffs().transpose() <<"\n"<<Qwb.toRotationMatrix() << std::endl;
    ...
    std::vector < std::pair< Eigen::Vector4d, Eigen::Vector4d > > lines;
    lines = CreatePointsLines(points);  //yzp: 将所有的点放入points里面，points里面的点都是唯一的， lines是从house里面读取的 一行中对应的两个点，应该是是匹配点???
